  ...
  *nivel++;
  ...
  for(int i = 0; i < pNo->U.Interno.qtdChaves; i++){
    if(pNo->U.Interno.chaves[i] > chave)
      ...
      return ArvorePercorre(pNo->U.Interno.apontadores[pNo->U.Interno.qtdChaves], chave);//se a chave for maior do que qualquer uma do nó, ela vai para o ultimo ponteiro do vetor
  }
...
bool arvorePesquisa(ArvoreBEstrela pNo, int chave, Registro* registro){//OK
