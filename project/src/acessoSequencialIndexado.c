...
bool criarPaginaIndicePrincipal(FILE* arquivo, Analise* analise, int quantidade) {

  FILE* arquivoPaginaIndexPrincipal = fopen("./arquivos/crescenteIndex.bin", "w+b");
  FILE* arquivoPaginaIndexPrincipalTxt = fopen("./arquivos/crescenteIndex.txt", "w+");
  if (arquivoPaginaIndexPrincipal == NULL) return false;

  Registro registro;
  int posicaoItemAtual = 0;
  int i = 0;
  while (fread(&registro, sizeof(Registro), 1, arquivo) == 1 && i < quantidade) {
...
bool buscaRegistroNaPagina(int chave, Registro* registro, ItemIndice indice, FILE* arquivoDeRegitros) {
  fseek(arquivoDeRegitros, sizeof(Registro) * indice.posicao, SEEK_SET);

  Registro registros[QTD_ITENS_A_SER_LIDOS];
  int qtdItem;
  while ((qtdItem = fread(registros, sizeof(Registro), QTD_ITENS_A_SER_LIDOS, arquivoDeRegitros)) > 0) {
...
bool pesquisaSequencial(int chave, Registro* registro,  FILE* arquivoDeRegistros, Analise* analise) {
  ...
  while (!fimDoArquivoIndexPrincipal) {
    ...
    int qtdItensLidos = fread(itensDoArquivoIndex, sizeof(ItemIndice), QTD_ITENS_A_SER_LIDOS, arquivoDeIndex);
    ...
    for (int i = 0; i < qtdItensLidos; i++) {
      ...
      if (chaveAtual == chave){
        return buscaRegistroNaPagina(chave, registro, itensDoArquivoIndex[i], arquivoDeRegistros);
      ...
      if (chaveAtual > chave) {
        ...
        if (ehPrimeiroItemDaPagina) {
          return buscaRegistroNaPagina(chave, registro, ultimoItemDaPagina, arquivoDeRegistros);
        }
        return buscaRegistroNaPagina(chave, registro,
                                     itensDoArquivoIndex[i - 1], arquivoDeRegistros);
      }
    }
...
Registro acessoSequencialIndexado(Analise* analise, int chave, short* achou, FILE* arquivo, int quantidade){
    ...
    valido = criarPaginaIndicePrincipal(arquivo, analise, quantidade);
