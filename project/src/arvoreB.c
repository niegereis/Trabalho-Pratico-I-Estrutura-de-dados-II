...
// void arvoreB(Analise *analise, FILE *arquivo, Registro *reg, int situacao, int quantidade, TipoApontador Arvore){
...
void InsereNaPagina (TipoApontador Ap, Registro Reg, TipoApontador ApDir, Analise *analise){
...
void Ins (Registro Reg, TipoApontador Ap, short *Cresceu, Registro *RegRetorno, TipoApontador *ApRetorno, Analise *analise){ 
    long i = 1; long j;
    ...
    if (i < M + 1){
        InsereNaPagina(ApTemp, Ap->r[2*M-1], Ap->p[2*M], analise); //Vai inserir o item que existe na página que existe. Os registros e ponteiros são distribuídos entre a página atual e a nova página temporária 
        Ap->n--;
...
Registro arvoreB(Analise* analise, int chave, short* achou, FILE* arquivo, int quantidade){
